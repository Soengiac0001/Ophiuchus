...
void adapt_environment() {
    ...
    memory[pointer] ^= (uint8_t)(rand() % 256);
}
...
void ai_echo_custom(const char* context, const char* message_hint) {
    (void)message_hint;              // currently unused, reserved for future
    srand((unsigned)time(NULL));    // explicit cast to match srand's signature
