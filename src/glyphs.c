...
void run_instruction(char instr) {
    ...
    switch (instr) {
        case '1': memory[pointer]++; break;
        case '2': memory[pointer]--; break;
        case '3': pointer = (pointer + 1) % MEM_SIZE; break;
        case '4': pointer = (pointer + MEM_SIZE - 1) % MEM_SIZE; break;
        case '5': {
            int ch = getchar();
            if (ch >= '0' && ch <= '9') {
                memory[pointer] = (uint8_t)(ch - '0');
            } else if (ch == 'A' || ch == 'a') {
                memory[pointer] = (uint8_t)10;
            } else if (ch == 'B' || ch == 'b') {
                memory[pointer] = (uint8_t)11;
            }
            while (getchar() != '\n');
        } break;
        case '6': printf("%X", (unsigned)memory[pointer]); break;
