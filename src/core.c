...
void poke_memory(int address, int value) {
    if (address >= 0 && address < MEM_SIZE) {
        ...
        if (normalized < 0) {
            normalized += 256;    // make sure we’re in 0..255 even for negative inputs
        }

        memory[address] = (uint8_t)normalized;
        printf("Poked memory[%d] = %u\n", address, (unsigned)memory[address]);
    } else {
